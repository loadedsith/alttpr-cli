...
const getCurrentRomHash = (path='./current_rom_hash.json', write=true) => {
  return new Promise((resolve, reject) => {
    getDaily().then((body) => {
      ...
      const hash = body.match(/current_rom_hash="([^"'\n;]*)/)[1]
...
const getPatch = (hash, saveAs, write=true) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    https.get(`https://s3.us-east-2.amazonaws.com/alttpr-patches/${hash}.json`,
      (response) => {
        if (response.statusCode == 200 && response.statusCode < 299) {
...
const getCurrentBasePatch = (path='./base_patch.json', write=true) => {
  return new Promise((resolve, reject) => {
    getDaily().then((body) => {
      ...
      body = body.find((line) => {
        return line.indexOf('override-base-bps=')> -1
      });

      const patch = body.match(/override-base-bps=([^"'\n;]*)/)[0];
      ...
      resolve(body.match(/override-base-bps=([^"'\n;]*)/)[0])
    }).catch(reject);
  })
};
